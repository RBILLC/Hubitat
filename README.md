...
- **Default transition (ms)** — how long a full brightness sweep takes when a command carries
  ...
  below about 500 the Bridge writes fewer of the halo's ten levels rather than slowing down.
  Blank sends nothing and the Bridge's own `transition_seconds` applies. A rate passed to
